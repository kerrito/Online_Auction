...
namespace Online_Auction.Pages.user_dashboard.bid_logs
{
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(constr))
                {
                    ...
                    string sql = "SELECT * FROM biding INNER JOIN users ON users.id=biding.buyer_id " +
                        "INNER JOIN product ON product.id=biding.item_id WHERE biding.buyer_id=@id ORDER BY biding.id DESC";
